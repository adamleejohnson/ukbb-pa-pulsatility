...
def joint_augmentation_subject(x, y, **kwargs):
    """Apply joint augmentations to multiple inputs of equal number of dimensions. 
       Channel-axis is NOT optional, and is expected to be the last axis.
    """

    angle_max    = kwargs.pop('angle_degree',360);
    pixel_max    = kwargs.pop('translate_pixels',50)
    ...
    if np.random.rand() < often_affine:
        ...
        x = rotate(x, angle=angle, axes=(2,3), reshape=False, order=1, mode='constant', cval=0)
        y = rotate(y, angle=angle, axes=(1,2), reshape=False, order=0, mode='constant', cval=0)
    
    # Random translations and inversions

    if np.random.rand() < often_affine:
        x = np.flip(x, axis=(2,3))
        y = np.flip(y, axis=(1,2))
    ...
    if crop:
        x = x[:,:,64:128+64, 64:128+64]
        y = y[:,64:128+64, 64:128+64]
